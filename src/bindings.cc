...
void SimulateFromXml(const Nan::FunctionCallbackInfo<Value>& args) {
    ...
    baton->options = new string(*Nan::Utf8String(args[0]->ToString(Nan::GetCurrentContext()).FromMaybe(v8::Local<v8::String>())));
    baton->files = new map<string, string>();

    Local<Object> files = args[1]->ToObject(Nan::GetCurrentContext()).FromMaybe(v8::Local<v8::Object>());
    Local<Array> props = files->GetPropertyNames(Nan::GetCurrentContext()).ToLocalChecked();
    for(int i = 0; i < props->Length(); i++) {
        if ( props->Get(Nan::GetCurrentContext(), i).FromMaybe(v8::Local<v8::Value>())->IsString()) {
            Local<String> key = props->Get(Nan::GetCurrentContext(), i).FromMaybe(v8::Local<v8::Value>())->ToString(Nan::GetCurrentContext()).FromMaybe(v8::Local<v8::String>());
            if (files->Get(Nan::GetCurrentContext(), key).FromMaybe(v8::Local<v8::Value>())->IsString()) {
                Nan::Utf8String val(files->Get(Nan::GetCurrentContext(), key).FromMaybe(v8::Local<v8::Value>())->ToString(Nan::GetCurrentContext()).FromMaybe(v8::Local<v8::String>()));
...
void RegisterEvent(const Nan::FunctionCallbackInfo<Value>& args) {
    ...
    string eventName = *Nan::Utf8String(args[0]->ToString(Nan::GetCurrentContext()).FromMaybe(v8::Local<v8::String>()));
...
void initOpenMM(Local<Object> exports,
    ...
    Nan::Set(exports,
        ...
        Nan::GetFunction(Nan::New<FunctionTemplate>(SimulateFromXml)).ToLocalChecked());
    Nan::Set(exports,
        Nan::New<String>("on").ToLocalChecked(),
        Nan::GetFunction(Nan::New<FunctionTemplate>(RegisterEvent)).ToLocalChecked());
    Nan::Set(exports,
        Nan::New<String>("continue").ToLocalChecked(),
        Nan::GetFunction(Nan::New<FunctionTemplate>(Continue)).ToLocalChecked());
    Nan::Set(exports,
        Nan::New<String>("pause").ToLocalChecked(),
        Nan::GetFunction(Nan::New<FunctionTemplate>(Pause)).ToLocalChecked());
    Nan::Set(exports,
    ...
    Nan::Set(exports,
        ...
        Nan::GetFunction(Nan::New<FunctionTemplate>(GetVersionName)).ToLocalChecked());
}
